...
interface SpecFinalCTAProps {
  onScrollToForm: () => void;
}

const SpecFinalCTA: React.FC<SpecFinalCTAProps> = ({ onScrollToForm }) => {
  return (
    <section className="bg-[#0F1115] py-16 md:py-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[#F5F7FA] mb-6">
          Ready to simplify{' '}
          <span className="text-[#BADF24]">spec material procurement?</span>
        </h2>
        <p className="text-xl text-[#B7C0CC] mb-10 max-w-2xl mx-auto">
          Get pricing and reserve delivery. We'll handle sourcing, logistics, and documentation.
        </p>
        ...
        <div className="mt-12 flex flex-wrap justify-center gap-6">
          {['Nationwide Coverage', 'Same-Day Response', 'Volume Pricing'].map(
            (badge, index) => (
              <span
                key={index}
