...
export async function calculateFinalPrice(
  ...
  try {
    ...
    // Get ZIP code adjustment if applicable (as a percentage, e.g., 10 for +10%)
    let zipAdjustment = 0;
    if (zipCode) {
    ...
    const priceWithZipAdjustment = baseWithMultiplier * (1 + zipAdjustment / 100);
    
    // Calculate the final per-ton price
    const pricePerTon = Math.round(priceWithZipAdjustment * 100) / 100;
